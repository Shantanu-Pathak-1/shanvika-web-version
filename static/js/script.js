...
let deferredPrompt; // PWA Prompt Store
...
async function sendMessage() { const input = document.getElementById('user-input'); const message = input.value.trim(); if (!message && !currentFile) return; if (abortController) { abortController.abort(); abortController = null; document.getElementById("loading-bubble")?.remove(); document.querySelector('button[type="submit"] i').className = "fas fa-arrow-up"; document.querySelector('button[type="submit"]').classList.remove("bg-red-500"); appendMessage('shanvika', "🛑 Stopped."); clearFile(); return; } if (!currentSessionId) { try { const res = await fetch('/api/new_chat'); const data = await res.json(); currentSessionId = data.session_id; loadHistory(); } catch (e) { return; } } document.getElementById('welcome-screen').style.display = 'none'; let displayMsg = message; if (currentFile) displayMsg += ` <br><span class="text-xs text-gray-400 bg-gray-800 px-2 py-1 rounded mt-1 inline-block"><i class="fas fa-file"></i> ${currentFile.name}</span>`; appendMessage('user', displayMsg); input.value = ''; abortController = new AbortController(); const btn = document.querySelector('button[type="submit"]'); const icon = btn.querySelector('i'); icon.className = "fas fa-stop"; btn.classList.add("bg-red-500"); let loadTxt = "Thinking..."; if (currentMode === 'image_gen') loadTxt = "🎨 Painting..."; else if (currentMode === 'converter') loadTxt = "🔄 Converting..."; const chatBox = document.getElementById('chat-box'); const loader = document.createElement('div'); loader.id = "loading-bubble"; loader.className = "p-4 mb-4 rounded-2xl bg-gray-800 w-fit mr-auto border border-gray-700 flex items-center gap-2"; loader.innerHTML = `<i class="fas fa-robot text-pink-500 animate-pulse"></i> <span class="text-gray-400 text-sm">${loadTxt}</span>`; chatBox.appendChild(loader); chatBox.scrollTop = chatBox.scrollHeight; try { const payload = { message: message, session_id: currentSessionId, mode: currentMode, file_data: currentFile?.data, file_type: currentFile?.type }; clearFile(); const res = await fetch('/api/chat', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload), signal: abortController.signal }); const data = await res.json(); loader.remove(); if (data.reply) { appendMessage('shanvika', data.reply); speakText(data.reply); } else appendMessage('shanvika', "⚠️ Empty response."); } catch (e) { loader.remove(); if (e.name !== 'AbortError') appendMessage('shanvika', "⚠️ Error."); } finally { abortController = null; icon.className = "fas fa-arrow-up"; btn.classList.remove("bg-red-500"); clearFile(); } }
